...
def main():
    ...
    player = (spark.read
              ...
              .where(~functions.col("totalFg3a").isNull())
              .where(~functions.col("totalFg3m").isNull())
              .repartition(8))
    ...
    pipelineCV = Pipeline(stages=[SQLTransformer(statement=sqlStatement), position_indexer, status_indexer,
                                    position_encoder, status_encoder, vectorAssembler, lr])
    # lrModelCV = pipelineCV.fit(train)
    lrModelCV = pipelineCV.fit(train)
    trainDF = lrModelCV.transform(train)
    trainDF.show()
    r2_train = r2_evaluator.evaluate(trainDF)
    print(f"r2 training for predicting nextSeasonAvgFg3p is {r2_train}")
    predDF = lrModelCV.transform(validation)
    r2 = r2_evaluator.evaluate(predDF)
    print(f"r2 validation for predicting nextSeasonAvgFg3p is {r2}")
