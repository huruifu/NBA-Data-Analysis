...
from pyspark.ml.regression import GBTRegressor, RandomForestRegressor, LinearRegression, DecisionTreeRegressor, GBTRegressor
...
def main():
    ...
    vectorAssembler = VectorAssembler(inputCols=["player_position_index",
                                                 "avgFgp",
                                                 "nextSeasonAge", "nextSeasonHeight", "nextSeasonWeight"],
                                      outputCol="features").setHandleInvalid("skip")
    ...
    (t0.totalFgm / t0.totalFga) AS avgFgp, t1.player_position, t1.age AS nextSeasonAge,
    LOG(t1.player_height) AS nextSeasonHeight, LOG(t1.player_weight) AS nextSeasonWeight,
    (t1.totalFgm / t1.totalFga) AS nextSeasonAvgFgp
    FROM __THIS__ as t0
    ...
    # grid = (ParamGridBuilder()
    #         .baseOn({rfr.labelCol: 'nextSeasonAvgFgp'})
    #         .baseOn([rfr.predictionCol, 'prediction'])
    #         # .addGrid(rfr.maxIter, [1, 5])
    #         .addGrid(rfr.maxDepth, [1, 8])
    #         .build())
    
    # dtr = DecisionTreeRegressor(featuresCol="features", labelCol="nextSeasonAvgFgp", maxDepth=8, varianceCol="variance")
    # grid = (ParamGridBuilder()
    #         .baseOn({dtr.labelCol: 'nextSeasonAvgFgp'})
    #         .baseOn([dtr.predictionCol, 'prediction'])
    #         # .addGrid(dtr.maxIter, [1, 5])
    #         .addGrid(dtr.maxDepth, [1, 8])
    #         .build())
    
    gbtr = GBTRegressor(featuresCol="features", labelCol="nextSeasonAvgFgp",
                       maxIter=5, maxDepth=5, seed=42)
    grid = (ParamGridBuilder()
            .baseOn({gbtr.labelCol: 'nextSeasonAvgFgp'})
