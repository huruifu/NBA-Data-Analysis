...
def main():
    ...
    player_schema = types.StructType([
        ...
        types.StructField('totalEfficiency', types.DoubleType()),
    ...
    player = (spark.read
            ...
            #   .withColumn("totalEfficiency", functions.col("totalEfficiency").cast(types.FloatType()))
              .repartition(8))
    ...
    # lr = LinearRegression(featuresCol="features", labelCol="nextSeasonTotalEfficiency", maxIter=4)
    ...
    # dtr = DecisionTreeRegressor(featuresCol="features", labelCol="nextSeasonTotalEfficiency", maxDepth=8,
    #                             varianceCol="variance")
    # grid = (ParamGridBuilder()
    #         .baseOn({dtr.labelCol: 'nextSeasonTotalEfficiency'})
    #         .baseOn([dtr.predictionCol, 'prediction'])
    #         # .addGrid(dtr.maxIter, [1, 5])
    #         .addGrid(dtr.maxDepth, [1, 8])
    #         .build())
    
    # rfr = RandomForestRegressor(featuresCol="features", labelCol="nextSeasonTotalEfficiency",
    #                     numTrees=5, maxDepth=8, seed=42, featureSubsetStrategy="all")
    # grid = (ParamGridBuilder()
    #         .baseOn({rfr.labelCol: 'nextSeasonTotalEfficiency'})
    #         .baseOn([rfr.predictionCol, 'prediction'])
    #         # .addGrid(rfr.maxIter, [1, 5])
    #         .addGrid(rfr.maxDepth, [1, 8])
    #         .build())
    
    gbtr = GBTRegressor(featuresCol="features", labelCol="nextSeasonTotalEfficiency",
                       maxIter=5, maxDepth=5, seed=42)
    grid = (ParamGridBuilder()
            .baseOn({gbtr.labelCol: 'nextSeasonTotalEfficiency'})
            .baseOn([gbtr.predictionCol, 'prediction'])
            .addGrid(gbtr.maxIter, [1, 5])
            .addGrid(gbtr.maxDepth, [1, 3])
            .build())
    
    cv = CrossValidator(estimator=gbtr, estimatorParamMaps=grid, evaluator=r2_evaluator, numFolds=10)
    ...
    print(f"r2 training for predicting nextSeasonTotalEfficiency is {r2_train}")
    predDF = lrModelCV.transform(validation)
    r2 = r2_evaluator.evaluate(predDF)
    print(f"r2 validation for predicting nextSeasonTotalEfficiency is {r2}")
    ...
    # plot_validation_result(predDF.select("prediction").collect(), predDF.select("nextSeasonTotalEfficiency").collect())    
    ...
    #                   "nextSeasonHeight", "nextSeasonWeight", "nextSeasonTotalEfficiency").toPandas()
    # plot_corr(df)
